...
#             else:
#                 allPossible[p] = allPossible[p] * noisyDistance
            else:
#             elif emissionModel[trueDistance] > 0:
                pacmanPosition = self.getJailPosition()
                trueDistance = util.manhattanDistance(p, pacmanPosition)
...
#                 self.setGhostPosition(gameState, newPos)
                newPosition = self.beliefs[oldPos] * float(prob)
                newBeliefs[newPos] += newPosition
...
#                 possiblePositions.append(itertools.product(p))
#         possiblePositions = [x for x in itertools.product(*[self.legalPositions for i in range(self.numGhosts)])]
        ...
        # each particle is a = tuple(ghost positions)
        # itertools.product for permutations of ghost positions
#         self.particles = []
#         posList = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
#         for i in range(self.numParticles):
#             self.particles = [tuple(posList[i % len(posList)])]
...
#                 posList.append(random.choice(self.legalPositions))
#             self.particles.append(tuple(self.particles))
...
#             particleWeights[particle] = 1.0
            ...
            for i in range(self.numGhosts):
                if not(noisyDistances[i] == None):
                    trueDistance = util.manhattanDistance(ghost[i], pacmanPosition)
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                    ...
                    for p in self.particles:
                        particlesNew.append(self.getParticleWithGhostInJail(p, i))
...
#                 newParticle = []
#                 for j in range(self.numGhosts):
#                     if not(noisyDistances[j] == None):
#                         newParticle.append(sample[j])
#                     else:
#                         newParticle.append(self.getJailPosition(j))
# #                         newParticle.append(self.getParticleWithGhostInJail(self.particles[i], j))                        
...
#             for particle in self.particles:
#                 if noisyDistances[i] == None:
#                     self.particles.append(self.getParticleWithGhostInJail(particle, i))
...
# #             particleWeights[particle] += weight
#         particleWeights.normalize()
#                     
