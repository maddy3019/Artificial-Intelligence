...
def enhancedFeatureExtractorDigit(datum):
    ...
    for x in range(DIGIT_DATUM_HEIGHT):
        enhFeature = 0
        pixelFrequency = 0
        enhPixel = othFeature = 0
        for y in range(DIGIT_DATUM_WIDTH):
            white = datum.getPixel(x, y) > 0
            grey = datum.getPixel(x, y) == 0
            if enhFeature == 0 and white:
                enhFeature = 1
            if enhPixel == 0 and (enhFeature == 1 and grey):
                enhPixel = 1
            if enhFeature == 1 and enhPixel == 1 and white:
                othFeature = 1
            if enhPixel == 1 and othFeature == 1 and (enhFeature == 1 and grey):
                pixelFrequency += 1
                enhFeature = 0
                enhPixel = 0
                othFeature = 0
    ...
    return features
    # breaks = 0
    # # row,column = datum.getPixels()
    # i = 0
    # nonzero = 0
    # firstLeft = None
    # aboveCenter = 0
    # # while i < len(pixels):
    # for i in range(len(datum.getPixels())):
    #     row = datum.getPixels()[i]
    #     active = False
    #     j = 1
    #     # while j < len(row):
    ...
    #     # i += 1
    #
    # width = len(datum.getPixels()[0]) - (firstLeft * 2)
    # j = 0
    # firstTop = None
    # pastRight = 0
    # # while j < len(datum.getPixel()[0]):
    # for j in range(len(datum.getPixels()[0])):
    #     active = False
    #     col = [p[j] for p in datum.getPixels()]
    #     i = 1
    #     # while i < len(col):
    ...
    #                 firstTop = i
    #             if i <= (len(datum.getPixels()[0]) + 1) / 2:
    #                 pastRight += 1
    #         if col[i] != row[i - 1]:
    #             breaks += 1
    #         # i += 1
    #     # j += 1
    #
    # height = len(datum.getPixels()) - (firstTop * 2)
...
def enhancedPacmanFeatures(state, action):
    ...
    for ghost in state.getGhostPositions():
        minDist = min(minDist, util.manhattanDistance(ghost, pacmanPos))
    features["closest ghost"] = minDist

    minDist = 9999
    for food in state.getFood().asList():
        minDist = min(minDist, util.manhattanDistance(food, pacmanPos))
    features["closest food"] = 1.0 / (minDist + .15)
    # if minDist == 9999:
    #     features["closest food"] = 2.5
    # else:
    #     features["closest food"] = 1.0 / (minDist + .15)

    return features
    # successorStates = state.generatePacmanSuccessor(action)
    # pacmanPos = successorStates.getPacmanPosition()
    # food = successorStates.getFood()
    # ghostStates = successorStates.getGhostStates()
    # scared = []
    # # if action == 'Stop':
    ...
    #
    # for ghost in state.getGhostPositions():
    #     ghostDist = [util.manhattanDistance(ghost, pacmanPos)]
    #     # scared.append(ghost.scaredTimer)
    #     # if ghost.scaredTimer == 0 and ghost.getPosition() == pos:
    #     #     features[action, 'val'] = -150
    #     #     return features
    # features["closest ghost"] = max(ghostDist)
    # for food in successorStates.getFood():
    #     foodDist = [util.manhattanDistance(food, pacmanPos)]
    #
    # k = 1.0 / (max(foodDist) + .15)
    # features["closest food"] = k
    # return features
